...
Следующим шагом нужно перенести соответствующие файлы на TypeScript. Самый первый шаг для вас - переименовать расширение файла с .js(x) на .ts(x). Это сообщит компилятору TypeScript, что это больше не JavaScript, а TypeScript и включит дополнительные синтаксические функции, такие как определение типов.
